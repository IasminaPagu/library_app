...
public class CartService {
    ...
    /**
     * Adaugă o carte în coș (sau crește cantitatea dacă există deja).
     */
    @Transactional
    public CartDto addToCart(Long userId, Long bookId, int qty) {
        ...
        // 4. Caută item-ul existent folosind Optional și Objects.equals()
        Optional<CartItem> existingOpt = cart.getItems().stream()
                .filter(i -> Objects.equals(i.getBook().getId(), bookId))
                .findFirst();

        if (existingOpt.isPresent()) {
            // 5a. Dacă există deja, crește cantitatea
            CartItem existing = existingOpt.get();
            existing.setQuantity(existing.getQuantity() + qty);
        } else {
            // 5b. Dacă nu există, creează un nou CartItem
        ...
        // 6. Salvează și returnează DTO-ul
